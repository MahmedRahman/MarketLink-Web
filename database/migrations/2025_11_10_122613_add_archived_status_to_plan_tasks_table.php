...
return new class extends Migration
{
        ...
        if ($driver === 'sqlite') {
            ...
            DB::statement("
                ...
                SELECT 
                    ...
                    CASE 
                        WHEN status IN ('todo', 'in_progress', 'review', 'done', 'archived') THEN status
                        ELSE 'todo'
                    END as status,
    ...
    public function down(): void
    {
        ...
        if ($driver === 'sqlite') {
            ...
            // Create new table without 'archived' status
            DB::statement("
                CREATE TABLE plan_tasks_new (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    monthly_plan_id INTEGER NOT NULL,
            ...
            // Copy data (excluding archived tasks or convert them to 'done')
            DB::statement("
                INSERT INTO plan_tasks_new 
                SELECT 
                    id, monthly_plan_id, goal_id, assigned_to, title, description,
                    CASE WHEN status = 'archived' THEN 'done' ELSE status END as status,
                    list_type, `order`, due_date, task_data, color, created_at, updated_at
                FROM plan_tasks
            ");
            ...
            // MySQL/MariaDB
            DB::statement("ALTER TABLE plan_tasks MODIFY COLUMN status ENUM('todo', 'in_progress', 'review', 'done') DEFAULT 'todo'");
        }
    }
};
