...
@section('page-title', 'إضافة خطة اشتراك جديدة')
        ...
        <form method="POST" action="{{ route('admin.plans.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" id="price_egp" name="price_egp" value="{{ old('price_egp') }}" step="0.01" min="0" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    @error('price_egp')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="duration_days" name="duration_days" value="{{ old('duration_days', 30) }}" min="1" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    @error('duration_days')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex gap-4 pt-4 border-t">
                <button type="submit" class="btn-primary">حفظ الخطة</button>
